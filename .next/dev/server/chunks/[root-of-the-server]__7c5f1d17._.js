...
"[project]/app/api/db/setup-relationships/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function createRelationship(baseUrl, token, baseId, relationship) {
    try {
        ...
        const table = tablesData?.list?.find((t)=>t.id === fromTableId);
        if (!table) {
            return {
                success: false,
        ...
        existingColumn = columns.find((col)=>col.column_name === relationship.fromColumn || col.title === relationship.fromColumn);
        // Если не нашли, пробуем найти по другим вариантам
        if (!existingColumn) {
            existingColumn = columns.find((col)=>col.column_name?.replace(/_/g, "") === relationship.fromColumn.replace(/_/g, ""));
        }
        // Если все еще не нашли, выводим список доступных колонок
        if (!existingColumn) {
            const availableColumns = columns.map((col)=>col.column_name || col.title).join(", ");
            return {
                success: false,
        ...
        if (!columnId || columnId === existingColumn.column_name || typeof columnId === 'string' && !columnId.match(/^[a-z0-9]+$/i)) {
            ...
            if (metaResponse1.ok) {
                ...
                const metaColumn1 = metaData1?.list?.find((col)=>col.column_name === relationship.fromColumn || col.title === relationship.fromColumn);
            ...
            if (!columnId || columnId === existingColumn.column_name) {
                ...
                if (metaResponse2.ok) {
                    ...
                    const metaColumn2 = metaData2?.list?.find((col)=>col.column_name === relationship.fromColumn || col.title === relationship.fromColumn);
                    if (metaColumn2) {
                        if (metaColumn2.id) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7c5f1d17._.js.map