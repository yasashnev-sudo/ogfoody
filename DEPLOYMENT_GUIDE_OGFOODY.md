# 🚀 Полная инструкция по деплою ogfoody.ru

## 📋 Содержание

1. [Подготовка](#подготовка)
2. [Настройка домена](#настройка-домена)
3. [Настройка сервера](#настройка-сервера)
4. [Первоначальный деплой](#первоначальный-деплой)
5. [Настройка автоматического деплоя](#настройка-автоматического-деплоя)
6. [Рабочий процесс](#рабочий-процесс)
7. [Обслуживание и мониторинг](#обслуживание-и-мониторинг)
8. [Troubleshooting](#troubleshooting)

---

## 🎯 Подготовка

### Что у вас уже есть:

- ✅ Домен: `ogfoody.ru` (купленный на reg.ru)
- ✅ Сервер: `5.129.194.168` (TimewebCloud)
- ✅ NocoDB: `https://noco.povarnakolesah.ru`
- ✅ Git репозиторий: `https://github.com/yasashnev-sudo/ogfoody`
- ✅ SSH доступ к серверу

### Файлы которые я создал:

- ✅ `.github/workflows/deploy.yml` - автодеплой через GitHub Actions
- ✅ `ecosystem.config.js` - конфигурация PM2
- ✅ `nginx-ogfoody.conf` - конфигурация Nginx
- ✅ `server-setup.sh` - скрипт автоматической настройки сервера
- ✅ `deploy-manual.sh` - скрипт для ручного деплоя
- ✅ `.env.production.example` - шаблон переменных окружения

---

## 🌐 Настройка домена

### Шаг 1: DNS на reg.ru

1. Зайдите в личный кабинет reg.ru
2. Найдите домен `ogfoody.ru`
3. Перейдите в "Управление DNS"
4. Добавьте A-записи:

```
Тип: A
Поддомен: @
IP: 5.129.194.168
TTL: 3600

Тип: A
Поддомен: www
IP: 5.129.194.168
TTL: 3600
```

5. Сохраните изменения
6. Подождите 5-30 минут пока DNS распространится

### Проверка DNS:

```bash
# С вашего компьютера
ping ogfoody.ru
# Должен отвечать 5.129.194.168

nslookup ogfoody.ru
# Должен показать правильный IP
```

---

## 🖥️ Настройка сервера

### Вариант А: Автоматическая настройка (Рекомендую!)

```bash
# 1. Подключитесь к серверу
ssh root@5.129.194.168

# 2. Загрузите скрипт настройки
wget https://raw.githubusercontent.com/yasashnev-sudo/ogfoody/main/server-setup.sh

# 3. Запустите
chmod +x server-setup.sh
bash server-setup.sh
```

Скрипт автоматически установит:
- Node.js 20.x
- PM2
- Nginx
- Certbot (для SSL)
- Настроит firewall
- Создаст директории

### Вариант Б: Ручная настройка

<details>
<summary>Развернуть подробные шаги</summary>

#### 1. Подключитесь к серверу

```bash
ssh root@5.129.194.168
```

#### 2. Обновите систему

```bash
apt update && apt upgrade -y
```

#### 3. Установите Node.js 20.x

```bash
curl -fsSL https://deb.nodesource.com/setup_20.x | bash -
apt install -y nodejs
node -v  # Проверка: должно быть v20.x.x
```

#### 4. Установите PM2

```bash
npm install -g pm2
pm2 startup systemd -u root --hp /root
```

#### 5. Установите Nginx

```bash
apt install -y nginx
systemctl enable nginx
systemctl start nginx
```

#### 6. Установите Certbot

```bash
apt install -y certbot python3-certbot-nginx
```

#### 7. Настройте firewall

```bash
ufw --force enable
ufw allow 'Nginx Full'
ufw allow OpenSSH
ufw allow 22/tcp
```

#### 8. Создайте директории

```bash
mkdir -p /var/www/ogfoody
mkdir -p /var/www/ogfoody/logs
mkdir -p /var/www/letsencrypt
chown -R www-data:www-data /var/www/letsencrypt
```

</details>

---

## 🚀 Первоначальный деплой

### Шаг 1: Клонирование проекта на сервер

```bash
# На сервере
cd /var/www/ogfoody
git clone https://github.com/yasashnev-sudo/ogfoody.git .
```

### Шаг 2: Настройка переменных окружения

```bash
# Создайте файл .env.production
nano /var/www/ogfoody/.env.production
```

Вставьте (замените на ваши реальные значения):

```env
NODE_ENV=production
PORT=3000

# NocoDB
NOCODB_URL=https://noco.povarnakolesah.ru
NOCODB_TOKEN=ваш_токен_из_nocodb

# Table IDs
NOCODB_TABLE_MEALS=mmtctn4flssh2ua
NOCODB_TABLE_EXTRAS=mksy21hmttmo855
NOCODB_TABLE_DELIVERY_ZONES=mpoppulqhsz1der
NOCODB_TABLE_USERS=mvrp4r9o3z69c45
NOCODB_TABLE_ORDERS=meddiicl0gr0r8y
NOCODB_TABLE_ORDER_PERSONS=mvr08d33zm5i8oi
NOCODB_TABLE_ORDER_MEALS=mz9uw5by177ygug
NOCODB_TABLE_ORDER_EXTRAS=mksy21hmttmo855
NOCODB_TABLE_PROMO_CODES=mgov8ce836696fy
NOCODB_TABLE_REVIEWS=mv8c69ib9muz9ki
```

Сохраните: `Ctrl+O`, `Enter`, `Ctrl+X`

### Шаг 3: Установка зависимостей и сборка

```bash
cd /var/www/ogfoody
npm install
npm run build
```

### Шаг 4: Запуск через PM2

```bash
pm2 start ecosystem.config.js
pm2 save
pm2 status
```

Приложение запущено на `http://localhost:3000`!

### Шаг 5: Настройка Nginx

#### 5.1 Временная конфигурация (для получения SSL)

```bash
cat > /etc/nginx/sites-available/ogfoody-temp.conf << 'EOF'
server {
    listen 80;
    server_name ogfoody.ru www.ogfoody.ru;
    
    location /.well-known/acme-challenge/ {
        root /var/www/letsencrypt;
    }
    
    location / {
        proxy_pass http://localhost:3000;
        proxy_set_header Host $host;
    }
}
EOF

ln -s /etc/nginx/sites-available/ogfoody-temp.conf /etc/nginx/sites-enabled/
nginx -t
systemctl reload nginx
```

#### 5.2 Получение SSL сертификата

```bash
certbot certonly --webroot \
  -w /var/www/letsencrypt \
  -d ogfoody.ru \
  -d www.ogfoody.ru \
  --agree-tos \
  --no-eff-email \
  --email admin@ogfoody.ru
```

#### 5.3 Полная конфигурация Nginx

```bash
# Удалите временную конфигурацию
rm /etc/nginx/sites-enabled/ogfoody-temp.conf

# Скопируйте полную конфигурацию
# (Сначала загрузите nginx-ogfoody.conf на сервер через scp)
cp nginx-ogfoody.conf /etc/nginx/sites-available/ogfoody.conf
ln -s /etc/nginx/sites-available/ogfoody.conf /etc/nginx/sites-enabled/

# Проверьте конфигурацию
nginx -t

# Если всё ОК, перезагрузите Nginx
systemctl reload nginx
```

### Шаг 6: Проверка

```bash
# Проверьте статус приложения
pm2 status

# Проверьте логи
pm2 logs ogfoody --lines 50

# Откройте в браузере
# https://ogfoody.ru
```

---

## ⚙️ Настройка автоматического деплоя

### Шаг 1: Настройка GitHub Secrets

1. Откройте репозиторий: https://github.com/yasashnev-sudo/ogfoody
2. Перейдите в **Settings** → **Secrets and variables** → **Actions**
3. Нажмите **New repository secret**

Добавьте следующие secrets:

| Name | Value | Описание |
|------|-------|----------|
| `SERVER_HOST` | `5.129.194.168` | IP сервера |
| `SERVER_USER` | `root` | Пользователь для SSH |
| `SERVER_PASSWORD` | `pULRoAvF@P-@4Y` | Пароль SSH (временно) |
| `SERVER_PATH` | `/var/www/ogfoody` | Путь к проекту |

### Шаг 2 (Опционально): Использование SSH ключа

**Рекомендую** использовать SSH ключ вместо пароля для безопасности:

#### На вашем компьютере:

```bash
# Создайте SSH ключ
ssh-keygen -t ed25519 -C "github-actions-ogfoody"

# Скопируйте публичный ключ на сервер
ssh-copy-id -i ~/.ssh/id_ed25519.pub root@5.129.194.168

# Выведите приватный ключ
cat ~/.ssh/id_ed25519
```

#### На GitHub:

1. Скопируйте содержимое приватного ключа
2. Добавьте secret `SERVER_SSH_KEY` с этим содержимым
3. Удалите secret `SERVER_PASSWORD`

### Шаг 3: Тестирование автодеплоя

```bash
# На вашем компьютере
cd /Users/sergejasasnev/Downloads/my-project\ \(1\)

# Сделайте тестовое изменение
echo "# Test auto-deploy" >> README.md

# Закоммитьте и запушьте
git add .
git commit -m "Test auto-deployment"
git push
```

Проверьте в GitHub:
- Перейдите в **Actions**
- Увидите запущенный workflow "Deploy to ogfoody.ru"
- Дождитесь завершения (2-3 минуты)
- Проверьте сайт: https://ogfoody.ru

---

## 🔄 Рабочий процесс (как делать доработки)

### Стандартный workflow:

```
1. Вы: "Добавь корзину товаров"
   ↓
2. Я (AI): Делаю изменения в коде
   ↓
3. Вы: git add . && git commit -m "Добавил корзину" && git push
   ↓
4. GitHub Actions: Автоматически деплоит
   ↓
5. Через 2-3 минуты: Изменения на ogfoody.ru ✨
```

### Пример сессии:

```bash
# В Cursor - говорите мне что нужно изменить
# Я делаю изменения...

# Потом вы:
git status                           # Проверяете что изменено
git add .                            # Добавляете изменения
git commit -m "Описание изменений"   # Коммитите
git push                             # Пушите

# Автоматически деплоится!
```

### Если нужен ручной деплой:

```bash
# Используйте скрипт
chmod +x deploy-manual.sh
./deploy-manual.sh
```

---

## 📊 Обслуживание и мониторинг

### Полезные команды PM2:

```bash
# Статус приложения
pm2 status

# Логи (в реальном времени)
pm2 logs ogfoody

# Последние 100 строк логов
pm2 logs ogfoody --lines 100

# Только ошибки
pm2 logs ogfoody --err

# Перезапуск
pm2 restart ogfoody

# Остановка
pm2 stop ogfoody

# Информация о процессе
pm2 info ogfoody

# Мониторинг в реальном времени
pm2 monit
```

### Полезные команды Nginx:

```bash
# Проверка конфигурации
nginx -t

# Перезагрузка конфигурации
systemctl reload nginx

# Перезапуск Nginx
systemctl restart nginx

# Статус
systemctl status nginx

# Логи доступа
tail -f /var/log/nginx/ogfoody_access.log

# Логи ошибок
tail -f /var/log/nginx/ogfoody_error.log
```

### Обновление SSL сертификата:

```bash
# Автоматически обновляется через cron, но можно вручную:
certbot renew

# Тест обновления (без реального обновления)
certbot renew --dry-run
```

### Проверка здоровья приложения:

```bash
# Health check
curl https://ogfoody.ru/api/health

# Диагностика
curl https://ogfoody.ru/api/diagnose
```

### Мониторинг ресурсов:

```bash
# Использование CPU и памяти
htop

# Использование диска
df -h

# Процессы Node.js
ps aux | grep node

# Сетевые соединения
netstat -tulpn | grep :3000
```

---

## 🔧 Troubleshooting

### Проблема 1: Сайт не открывается

**Проверка:**

```bash
# Пингуется ли домен?
ping ogfoody.ru

# Работает ли Nginx?
systemctl status nginx

# Работает ли приложение?
pm2 status

# Доступен ли сайт локально?
curl http://localhost:3000
```

**Решения:**

```bash
# Перезапустить Nginx
systemctl restart nginx

# Перезапустить приложение
pm2 restart ogfoody

# Проверить логи
pm2 logs ogfoody --lines 50
tail -f /var/log/nginx/ogfoody_error.log
```

### Проблема 2: 502 Bad Gateway

**Причина:** Приложение не запущено или не отвечает на порту 3000

**Решение:**

```bash
# Проверьте что приложение запущено
pm2 status

# Если не запущено
cd /var/www/ogfoody
pm2 start ecosystem.config.js

# Проверьте логи ошибок
pm2 logs ogfoody --err
```

### Проблема 3: Товары не загружаются

**Причина:** Проблема с подключением к NocoDB

**Решение:**

```bash
# Проверьте .env.production
cat /var/www/ogfoody/.env.production

# Проверьте доступность NocoDB
curl -I https://noco.povarnakolesah.ru

# Проверьте диагностику
curl https://ogfoody.ru/api/diagnose

# Проверьте логи приложения
pm2 logs ogfoody | grep -i nocodb
```

### Проблема 4: SSL сертификат не работает

**Решение:**

```bash
# Проверьте сертификат
certbot certificates

# Обновите сертификат
certbot renew

# Проверьте конфигурацию Nginx
nginx -t

# Перезагрузите Nginx
systemctl reload nginx
```

### Проблема 5: GitHub Actions деплой не работает

**Проверка:**

1. Откройте https://github.com/yasashnev-sudo/ogfoody/actions
2. Посмотрите логи последнего workflow
3. Проверьте что все secrets настроены

**Типичные ошибки:**

- `Permission denied` → Проверьте SSH ключ или пароль в secrets
- `Port 22: Connection refused` → Проверьте firewall на сервере
- `npm run build failed` → Проверьте ошибки сборки в логах

### Проблема 6: Не хватает места на диске

**Решение:**

```bash
# Проверьте использование диска
df -h

# Очистите логи PM2
pm2 flush

# Очистите кеш npm
npm cache clean --force

# Удалите старые бэкапы
cd /var/www/backups
ls -t ogfoody_backup_*.tar.gz | tail -n +6 | xargs rm

# Очистите логи Nginx (старше 7 дней)
find /var/log/nginx -name "*.log" -type f -mtime +7 -delete
```

### Проблема 7: Приложение падает после деплоя

**Решение:**

```bash
# Откатитесь на предыдущую версию
cd /var/www/ogfoody
git log --oneline -10  # Найдите предыдущий коммит
git reset --hard COMMIT_HASH
npm run build
pm2 restart ogfoody

# Или восстановите из бэкапа
cd /var/www/backups
ls -lt  # Найдите последний бэкап
tar -xzf ogfoody_backup_TIMESTAMP.tar.gz -C /var/www/ogfoody
cd /var/www/ogfoody
pm2 restart ogfoody
```

---

## 📱 Быстрый справочник команд

### Деплой:

```bash
# Автоматический (рекомендуется)
git push

# Ручной
./deploy-manual.sh

# На сервере вручную
cd /var/www/ogfoody && git pull && npm install && npm run build && pm2 restart ogfoody
```

### Мониторинг:

```bash
# Статус всего
pm2 status && systemctl status nginx

# Логи в реальном времени
pm2 logs ogfoody

# Проверка сайта
curl -I https://ogfoody.ru
```

### Откат изменений:

```bash
# Через Git
cd /var/www/ogfoody
git log --oneline -10
git reset --hard COMMIT_HASH
npm run build && pm2 restart ogfoody

# Через бэкап
cd /var/www/backups
tar -xzf ogfoody_backup_TIMESTAMP.tar.gz -C /var/www/ogfoody
pm2 restart ogfoody
```

---

## 🔐 Рекомендации по безопасности

### 1. Смените пароль root

```bash
passwd
```

### 2. Создайте отдельного пользователя

```bash
adduser ogfoody
usermod -aG sudo ogfoody
```

### 3. Настройте SSH ключи

```bash
# На вашем компьютере
ssh-keygen -t ed25519
ssh-copy-id root@5.129.194.168

# На сервере отключите пароль SSH
nano /etc/ssh/sshd_config
# Измените: PasswordAuthentication no
systemctl restart sshd
```

### 4. Обновите Secrets в GitHub

После настройки SSH ключей обновите `SERVER_SSH_KEY` и удалите `SERVER_PASSWORD`.

### 5. Регулярно обновляйте систему

```bash
apt update && apt upgrade -y
```

---

## 📞 Поддержка

### Проверка состояния:

- **Сайт:** https://ogfoody.ru
- **Health check:** https://ogfoody.ru/api/health
- **Диагностика:** https://ogfoody.ru/api/diagnose

### Логи:

- **PM2:** `pm2 logs ogfoody`
- **Nginx access:** `/var/log/nginx/ogfoody_access.log`
- **Nginx errors:** `/var/log/nginx/ogfoody_error.log`

### GitHub Actions:

- **Workflows:** https://github.com/yasashnev-sudo/ogfoody/actions
- **Secrets:** https://github.com/yasashnev-sudo/ogfoody/settings/secrets/actions

---

## ✅ Checklist перед запуском

- [ ] DNS A-запись настроена (`ogfoody.ru` → `5.129.194.168`)
- [ ] Node.js 20.x установлен на сервере
- [ ] PM2 установлен
- [ ] Nginx установлен
- [ ] Проект склонирован в `/var/www/ogfoody`
- [ ] Файл `.env.production` создан с правильными значениями
- [ ] Проект собран (`npm run build`)
- [ ] Приложение запущено через PM2
- [ ] Nginx конфигурация скопирована и активирована
- [ ] SSL сертификат получен через certbot
- [ ] Сайт открывается по HTTPS
- [ ] GitHub Secrets настроены
- [ ] Автодеплой протестирован

---

## 🎉 Готово!

После выполнения всех шагов у вас будет:

- ✅ Рабочий сайт на https://ogfoody.ru
- ✅ Автоматический деплой при `git push`
- ✅ SSL сертификат (HTTPS)
- ✅ Автоперезапуск при падении
- ✅ Логирование
- ✅ Бэкапы перед каждым деплоем

**Теперь рабочий процесс прост:**

1. Говорите мне что нужно изменить
2. Я делаю изменения
3. Вы делаете `git push`
4. Через 2-3 минуты изменения на ogfoody.ru!

Удачи! 🚀

