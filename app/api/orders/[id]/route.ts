    ...
    if (order) {
      ...
      if (!wasCancelled && willBeCancelled && currentOrder.user_id) {
        try {
          ...
          if (wasPaid) {
            ...
            // Неоплаченный заказ - возвращаем использованные баллы (если были) и отменяем pending транзакции
            // ✅ ИСПРАВЛЕНО: Для неоплаченных заказов также возвращаем использованные баллы
            if (pointsUsed > 0) {
              console.log(`💰 Неоплаченный заказ ${id} имел использованные баллы (${pointsUsed}), возвращаем их`)
              await refundLoyaltyPoints(
                currentOrder.user_id,
                0, // Начисленных баллов нет (они были pending)
                pointsUsed, // Возвращаем использованные баллы
                orderTotal,
                Number(id)
              )
              console.log(`✅ Возвращено ${pointsUsed} использованных баллов для неоплаченного заказа ${id}`)
            }
      ...
      if (!wasCancelled && willBeCancelled && currentOrder.user_id) {
        try {
          ...
          // ✅ ИСПРАВЛЕНО: Для оплаченных заказов возвращаем использованные баллы и списываем начисленные
          // Для неоплаченных заказов также возвращаем использованные баллы (если были)
          if (wasPaid) {
            // Оплаченный заказ - возвращаем баллы (отменяем начисление и возвращаем использованные)
            if (pointsEarned > 0 || pointsUsed > 0) {
              await refundLoyaltyPoints(
                currentOrder.user_id,
                pointsEarned,
          ...
          } else if (!wasPaid && (pointsEarned > 0 || pointsUsed > 0)) {
            // Неоплаченный заказ - возвращаем использованные баллы (если были) и списываем начисленные (если были)
            await refundLoyaltyPoints(
              currentOrder.user_id,
              pointsEarned,
              pointsUsed,
            ...
            console.log(`✅ Неоплаченный заказ ${id} отменен: возвращено ${pointsUsed} использованных баллов, списано ${pointsEarned} начисленных`)
          }
        } catch (error) {
    ...
    if (!wasCancelled && currentOrder.user_id) {
      try {
        ...
        if (wasPaid) {
          ...
          // Заказ НЕ был оплачен - проверяем pending транзакции и использованные баллы
          const pendingTransactions = await fetchPendingTransactionsByOrder(Number(id))
          console.log(`🔍 Заказ НЕ был оплачен. Найдено pending транзакций: ${pendingTransactions.length}`)
          
          // ✅ ИСПРАВЛЕНО: Для неоплаченных заказов также возвращаем использованные баллы
          // (если они были использованы при создании заказа)
          if (pointsUsed > 0) {
            console.log(`💰 Неоплаченный заказ ${id} имел использованные баллы (${pointsUsed}), возвращаем их`)
            await refundLoyaltyPoints(
              currentOrder.user_id,
          ...
          if (pendingTransactions.length > 0) {
            // Отменяем pending транзакции (начисление баллов, которые еще не были начислены)
