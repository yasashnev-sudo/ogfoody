...
/**
 * Скрипт для добавления поля usage_type в таблицу Promo_Codes
 */
...
async function main() {
  console.log('🔧 Добавление поля usage_type в таблицу Promo_Codes...\n');
  ...
  try {
    ...
    // Добавляем usage_type если нет
    if (!columnNames.includes('usage_type')) {
      console.log('➕ Добавление колонки usage_type...');
      await addColumn({
      ...
      console.log('✅ Колонка usage_type добавлена!');
    } else {
      console.log('✅ Колонка usage_type уже существует');
    }

    console.log('\n✅ Таблица Promo_Codes обновлена!');
    console.log('\n💡 Теперь можно использовать типы использования:');
    console.log('   - unlimited (постоянно)');
    console.log('   - once_per_user (один раз на пользователя)');
    console.log('   - once_total (всего один раз)');

  } catch (error) {
    console.error('❌ Ошибка:', error.message);
    console.log('\n💡 Добавьте колонку вручную в NocoDB:');
    console.log('   - usage_type (Text, по умолчанию: "unlimited")');
    process.exit(1);
  }
}
